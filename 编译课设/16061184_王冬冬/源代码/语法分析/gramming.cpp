...
void err_mes(int n) {
	...
	case 32:
		...
		break;
	case 33:
		printf("Line %d:Error--------Array transboundary!\n", line);//数组越界
		break;
	case 34:
		printf("Line %d:Error--------The index of array isn't integer!\n", line);//数组下标不为整型
		break;
	case 35:
		printf("Line %d:Error--------The identifer is keyword!\n", line);//标识符为关键字
		break;
	case 36:
		printf("Line %d:Error--------The type of condition is mistake!\n", line);//条件类型错误
		break;
	}
}//错误信息
int tra_num(int a, char b[]) {
...
int look_value(char sym_name[]) {
	int num, i, j;
	num = tab_num - 1;
	for (i = num; i >= 0; i--) {
		if (!strcmp(sym_tab[i].name, sym_name)) {
			return sym_tab[i].num;
		}
		if (sym_tab[i].type == re_func_int || sym_tab[i].type == re_func_char || sym_tab[i].type == no_func) {
			for (j = 0; j<tab_num; j++) {
				if (!strcmp(sym_tab[j].name, sym_name) && sym_tab[j].level == 0) {
					return sym_tab[j].num;
				}
			}
			return -1;
		}
...
void statement() {
	...
	if (m == 5) {
		...
		if (m == 19) {//左小括号
			...
			index_type = 0;
			expression();
			if (index_flag == 1 && index_type == 1) {
				err_mes(34);//数组下标不为整型
			}
			if (array_flag == 1) {
				if (array_value >= look_value(assign_temp) || array_value < 0) {
					err_mes(33);//数组越界
				}
			}
			array_flag = 0;
...
void factor() {
	...
	if (m == 32) {
		...
		if (m == 21) {//'['
			...
			index_type = b;
			if (array_flag == 1) {
				if (array_value >= look_value(factor_temp) || array_value < 0) {
					err_mes(33);//数组越界
				}
			}
			array_flag = 0;
