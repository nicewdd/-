	...
	for (i = 0; i < mid_num; i++) {
		if (mid_tab[i].id == 9 || mid_tab[i].id == 10 || mid_tab[i].id == 11) {
			...
			if (mid_tab[i].id == 3 || mid_tab[i].id == 4) {
				...
				var_addr += 4;
				enter_var_tab(mid_tab[i].argu1, mid_tab[i].id, var_addr);
				strcpy(func_info[func_num - 1].para_name[para_num], mid_tab[i].argu1);
				para_num++;
			}
		}
		else {
...
void goal_code(int mid_num, int sym_num) {
	...
	for (i = 0; i < func_num; i++) {
		...
		printf("%d\n", func_info[i].var_num);
		for (j = 0; j < func_info[i].var_num; j++) {
			printf("%s    type:%d    addr:%d      quote:%d\n", func_info[i].var_info[j].name, func_info[i].var_info[j].type, func_info[i].var_info[j].addr, func_info[i].var_info[j].quote);
		}
	}
	fprintf(goal, ".data\n");
