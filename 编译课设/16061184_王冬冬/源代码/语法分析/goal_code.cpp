	...
	for (i = 0; i < mid_num; i++) {
		if (mid_tab[i].id == 9 || mid_tab[i].id == 10 || mid_tab[i].id == 11) {
			...
			if (func_num > 0) {
				...
				func_info[func_num - 1].final_addr = func_info[func_num - 1].var_info[var_num - 1].addr;
			}
			...
			if (mid_tab[i].id == 3 || mid_tab[i].id == 4) {
				...
				func_info[func_num - 1].var_info[var_num].front_addr = var_addr;
...
void quote_count() {
	...
	char func_name[130];
	for (i = 0; i < mid_num; i++) {
		pos = func_pos(func_name);
		if (mid_tab[i].id == 9 || mid_tab[i].id == 10 || mid_tab[i].id == 11) {
			strcpy(func_name, mid_tab[i].result);
		}
		else if (mid_tab[i].id >= 12 && mid_tab[i].id != 37) {
			for (j = 0; j < func_info[pos].var_num; j++) {
				if (!strcmp(mid_tab[i].argu1, func_info[pos].var_info[j].name)) {
					func_info[pos].var_info[j].quote++;
				}
				if (!strcmp(mid_tab[i].argu2, func_info[pos].var_info[j].name)) {
					func_info[pos].var_info[j].quote++;
				}
				if (!strcmp(mid_tab[i].result, func_info[pos].var_info[j].name)) {
					func_info[pos].var_info[j].quote++;
				}
			}
		}
...
void sort_var_info() {
	int i, j, k, m, temp_quote, temp_type, temp_array, temp_addr, temp_front;
	char temp_name[130];
	for (i = 0; i < func_num; i++) {
		k = func_info[i].var_num;
		for (m = 0; m < k - 1; m++) {
			for (j = k - 1; j > m; j--) {
				if (func_info[i].var_info[j - 1].quote < func_info[i].var_info[j].quote) {
					strcpy(temp_name, func_info[i].var_info[j - 1].name);
					strcpy(func_info[i].var_info[j - 1].name, func_info[i].var_info[j].name);
					strcpy(func_info[i].var_info[j].name, temp_name);
					temp_quote = func_info[i].var_info[j - 1].quote;
					func_info[i].var_info[j - 1].quote = func_info[i].var_info[j].quote;
					func_info[i].var_info[j].quote = temp_quote;
					temp_type = func_info[i].var_info[j - 1].type;
					func_info[i].var_info[j - 1].type = func_info[i].var_info[j].type;
					func_info[i].var_info[j].type = temp_type;
					temp_array = func_info[i].var_info[j - 1].array_num;
					func_info[i].var_info[j - 1].array_num = func_info[i].var_info[j].array_num;
					func_info[i].var_info[j].array_num = temp_array;
					temp_addr = func_info[i].var_info[j - 1].addr;
					func_info[i].var_info[j - 1].addr = func_info[i].var_info[j].addr;
					func_info[i].var_info[j].addr = temp_addr;
					temp_front = func_info[i].var_info[j - 1].front_addr;
					func_info[i].var_info[j - 1].front_addr = func_info[i].var_info[j].front_addr;
					func_info[i].var_info[j].front_addr = temp_front;
				}
			}
		}
	}
...
void creat_text(int mid_num, FILE *goal) {
	...
	for (i = 0; i < mid_num; i++) {
		...
		case 9:
			...
			sp_addr = func_info[j].final_addr;
		...
		case 29:
			...
			if (h >= 0) {
				if (h == 0) {
					...
					array_addr = func_info[func_position].var_info[h].front_addr;
				}
				if (k >= 0 && k < 8) {
			...
			if (k >= 0) {
				if (k == 0) {
					...
					if (s1 == 1) {
						...
						fprintf(goal, "\tadd $t4, $t3, %d\n", func_info[func_position].var_info[k].front_addr);
					}
					else {
		...
		case 43:
			...
			if (h >= 0 && h < 8) {
				if (k < 8) {
					...
					fprintf(goal, "\tsw $s%d, -%d($fp)\n", h, addr_var(curr_func, mid_tab[i].result) - 4);
				}
			}
			else if (h >= 8) {
				if (k < 8) {
					fprintf(goal, "\tlw $s%d, -%d($fp)\n", k, addr_var(curr_func, mid_tab[i].argu1) - 4);
				}
				else {
					fprintf(goal, "\tlw $t0, -%d($fp)\n", addr_var(curr_func, mid_tab[i].argu1) - 4);
					fprintf(goal, "\tsw $t0, -%d($fp)\n", addr_var(curr_func, mid_tab[i].result) - 4);
...
void goal_code(int mid_num, int sym_num) {
	...
	/*for (i = 0; i < func_num; i++) {
		...
		printf("%d\n", func_info[i].var_num);
		printf("%d\n", func_info[i].final_addr);
		for (j = 0; j < func_info[i].var_num; j++) {
			printf("%s    type:%d    addr:%d    quote:%d      front:%d\n", func_info[i].var_info[j].name, func_info[i].var_info[j].type, func_info[i].var_info[j].addr,func_info[i].var_info[j].quote, func_info[i].var_info[j].front_addr);
		}
	}*/
	fprintf(goal, ".data\n");
